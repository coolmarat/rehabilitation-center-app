...
import 'package:rehabilitation_center_app/core/database/app_database.dart' as db; 
import 'package:rehabilitation_center_app/features/activity_types/domain/activity_type.dart' as domain_activity_type;
import 'package:rehabilitation_center_app/features/clients/domain/child.dart' as domain_child;
import 'package:rehabilitation_center_app/features/employees/domain/employee.dart' as domain_employee;
...
class ScheduleRepositoryImpl implements ScheduleRepository {
  ...
  Future<void> addSession(Session session) async {
    final companion = db.SessionsCompanion.insert(
      ...
      durationMinutes: session.duration.inMinutes, // Use raw value for non-nullable
      price: session.price, // Use raw value for non-nullable
      isCompleted: Value(session.isCompleted), // Use Value() for field with default
      notes: session.notes == null ? const Value.absent() : Value(session.notes!), // Handle nullable
    );
  ...
  Future<ScheduleFormData> getScheduleFormData() async {
    ...
    final employees = (results[0] as List<db.EmployeeEntry>)
        .map((e) => domain_employee.Employee(
              id: e.id,
    ...
    final activityTypes = (results[1] as List<db.ActivityTypeEntry>)
        .map((at) => domain_activity_type.ActivityType(
              id: at.id,
              name: at.name,
    ...
    final children = (results[2] as List<db.ChildEntry>)
        .map((c) => domain_child.Child(
              id: c.id,
              parentId: c.parentId,
  ...
  Future<List<SessionDetails>> getSessionsForDay(DateTime date) async {
    ...
    final query = _database.select(_database.sessions).join([
      innerJoin(_database.children, _database.children.id.equalsExp(_database.sessions.childId)),
      innerJoin(_database.employees, _database.employees.id.equalsExp(_database.sessions.employeeId)),
      innerJoin(_database.activityTypes,
          _database.activityTypes.id.equalsExp(_database.sessions.activityTypeId)),
    ])
      ..where(_database.sessions.sessionDateTime.isBetweenValues(dayStart, dayEnd))
      ..orderBy([OrderingTerm(expression: _database.sessions.sessionDateTime)]);
  ...
  Future<void> updateSession(Session session) async {
    ...
    await (_database.update(_database.sessions)..where((tbl) => tbl.id.equals(session.id))).write(companion);
  }

  @override
  Future<int> deleteSession(int sessionId) async {
    return await (_database.delete(_database.sessions)..where((tbl) => tbl.id.equals(sessionId))).go();
  }
}
