                ...
                switch (state.status) {
                  ...
                  case ClientStatus.success:
                    if (state.parentsWithChildren.isEmpty) {
                      return const Center(child: Text('Нет клиентов. Добавьте первого клиента.'));
                    }
                    final parentTiles = state.parentsWithChildren.entries.map((entry) {
                      final parent = entry.key;
                      final children = entry.value;
                      // Получаем сет ID для разворота из состояния
                      final shouldExpand = state.initiallyExpandedParentIds.contains(parent.id);
                        ...
                        children: [
                          ...
                          if (children.isEmpty)
                            ...
                            ...children.map((child) => ListTile(
                                  ...
                                  subtitle: Text('Дата рожд.: ${child.dateOfBirth.toLocal().toString().split(' ')[0]}'), 
                                  trailing: Row(
                                    ...
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          showChildFormDialog(context, parentId: parent.id, child: child); 
                                        },
                                      ),
                                      IconButton(
                                        ...
                                        onPressed: () async {
                                          ...
                                          final confirmed = await showConfirmationDialog(
                                            ...
                                            content: 'Вы уверены, что хотите удалить ребенка "${child.fullName}"?',
                                            confirmText: 'Удалить',
                                          );
                                          if (confirmed == true) {
                                            context.read<ClientBloc>().add(DeleteChildRequested(child.id));
                                          }
                                        },
                                      ),
                                    ],
                          ...
                          ListTile(
                            ...
                            onTap: () {
                              ...
                              showChildFormDialog(context, parentId: parent.id);
                            },
                          ),
                        ],
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () {
                                // Используем context из Builder
                                showParentFormDialog(context, parent: parent); 
                              },
                            ),
                            IconButton(
                              icon: const Icon(Icons.delete, size: 20, color: Colors.redAccent),
                              ...
                              onPressed: () async {
                                ...
                                final confirmed = await showConfirmationDialog(
                                  ...
                                  content: 'Вы уверены, что хотите удалить родителя "${parent.fullName}"? Это действие также удалит всех связанных с ним детей.',
                                  confirmText: 'Удалить',
                                );
                                if (confirmed == true) {
                                  context.read<ClientBloc>().add(DeleteParentRequested(parent.id));
                                }
                              },
                            ),
                          ],
